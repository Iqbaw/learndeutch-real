...
import { ArrowRight, BookOpen, Flame, Trophy, Mic, RefreshCw, ChevronRight, Clock, Check, Sparkles, Map, ArrowUpRight, GraduationCap, ClipboardCheck } from "lucide-react";
  ...
  return <AppGuard><AppShell title="Ruang belajarmu" subtitle="Sedikit setiap hari, semakin percaya diri.">
    <div className="learning-home">
      ...
      <div className="home-layout"><div className="home-main">
        ...
        <section className="card-base mt-5 p-5"><span className="eyebrow">JALUR KURSUS & UJIAN</span><h2 className="mt-2 font-heading text-lg font-extrabold text-ink">Pelajari materi, lalu uji tiap keterampilan</h2><p className="mt-1 text-sm leading-6 text-muted">Setelah fondasi A1–A2, kursus B1–C2 menghubungkan grammar, kosakata, Lesen, Hören, Schreiben, dan Sprechen. Latihan ujian memakai tugas baru per modul.</p><div className="mt-4 flex flex-wrap gap-2"><CTAButton href={`/course?level=${["B1", "B2", "C1", "C2"].includes(activeLevel) ? activeLevel : "B1"}`} variant="outline"><GraduationCap size={17} /> Buka kursus</CTAButton><CTAButton href={`/exam?level=${["B1", "B2", "C1", "C2"].includes(activeLevel) ? activeLevel : "B1"}`} variant="outline"><ClipboardCheck size={17} /> Latihan ujian</CTAButton></div></section>
