...
export default function OnboardingPage() {
  const { profile, onboarding, setOnboardingAnswer, setPlacement, completeOnboarding, setDailyTarget, skipToLevel } =
    useAppStore();
  ...
  useEffect(() => {
    if (profile && phase === "profile") setPhase("placementIntro");
  }, [profile, phase]);
  ...
  async function finishPlacement(state: PlacementState) {
    ...
    const { summary, focusAreas } = await fetchPlacementSummary({
      ...
      profile: {
        ...
        weakSkill: profile?.weakSkill ?? onboarding.weakSkill,
  ...
  function finishOnboarding(startDay: number, startLevel: Profile["startLevel"]) {
    ...
    completeOnboarding(
      {
        name: profile?.name ?? ((onboarding.name ?? name).trim() || "Pelajar"),
        goal: profile?.goal ?? onboarding.goal ?? "Hobi",
        startLevel,
        weakSkill: profile?.weakSkill ?? onboarding.weakSkill ?? "Belum tahu",
        learningStyle: profile?.learningStyle ?? onboarding.learningStyle ?? "Campuran",
